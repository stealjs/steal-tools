...
function makeNpmHackNode(npmContext) {
	var packages = npmContext.packages;

	return {
		deps: [],
		load: {
			name: "set-npm-context-paths",
			metadata: {
				format: "global"
			},
			source: [
				"var npmPackages = " + JSON.stringify(packages) + ";",
				"for (var p of npmPackages) {",
				"\tvar path = p.origFileUrl.replace('file:', '.');",
				"\tSystem.npmContext.paths[path] = p;",
				"}"
			].join("\n")
		}
	};
...
function bundle(data) {
	...
	splitMainBundles.filter(isJSBundle).forEach(function(bundle) {
		...
		// HACK: push a node to the front of the bundle nodes that populates
		// `System.npmContent.paths` to prevent the npm extension to load
		// the package.json of the app dependencies when the dev bundle has
		// been loaded.
		if (npmContext) {
			unshift.apply(bundle.nodes, [
				makeNpmHackNode(npmContext)
			]);
		}
