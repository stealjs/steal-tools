...
var makeBundleGraph = module.exports = function(config, options){
	...
	var originalMains = Array.isArray(cfg.main) ? cfg.main : [cfg.main];
	...
	return dependencyGraph(cfg, options)
		...
		var getNextGraphAndMerge = function(){
			...
			if(!nextBundle) {
				var normalizedPromises = originalMains.map(function(main){
					return Promise.resolve(loader.normalize(main));
				});

				// If there are no more bundles, return data
				return Promise.all(normalizedPromises).then(function(mains) {
					return {
						graph: masterGraph,
						steal: data.steal,
						loader: data.loader,
				...
				return dependencyGraph(copy, options).then(function(data){
					mergeIntoMasterAndAddBundle(masterGraph, data.graph, data.steal.System.main);
