	...
	for(let exp of result.exports) {
		if(exp.star) {
			for(let lexp of recurseStar(node, exp.specifier)) {
				let exportName = lexp.exportName;
				edge.exports.set(exportName, Object.assign({exportName}, exp));

				let moduleName = getDependencyName(node, exp.specifier);
				getEdge(moduleName, map).usedExports.add(exportName);
			}
		} else {
			...
			if(exp.specifier) {
				let moduleName = getDependencyName(node, exp.specifier);
				getEdge(moduleName, map).usedExports.add(exp.exportName);
			}
		}
	}
