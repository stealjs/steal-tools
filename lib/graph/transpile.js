...
var transpileNode = function(node, outputFormat, options, graph, loader){
	...
	if( !node.load.metadata.buildType || node.load.metadata.buildType === "js"  ) {
		...
		transformActiveSource(node,key(options, outputFormat),function(node, source){
			...
			// By default Babel uses module.exports for a single export rather
			// than use exports like Traceur. This leads to inconsist output
			// dependending on which transpiler is used, so we fix it here.
			if(opts.transpiler === "babel") {
				var babelFormats = { cjs: "commonStrict", amd: "amdStrict" };
				var babelOpts = babelFormats[outputFormat] ? {
					modules: babelFormats[outputFormat]
				} : {};
				console.log("BABELOPTS:", babelOpts);
				opts.babelOptions = _.assign({}, babelOpts, loader.babelOptions);
			}
			console.log("TFORMAT:", opts.format);
