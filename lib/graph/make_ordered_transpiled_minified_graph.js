...
var makeGraph = require("../graph/make_graph"),
	...
	eachGraph = require("../graph/each_dependencies"),
	minifyGraph = require("../graph/minify"),
	makeDependencyMap = require("../node/make_dependency_map");

module.exports = function(config, options){
	// An option for whether we should map our dependencies and pass this information
	// into transpile
	var mapDependencies = options && options.dependencyMapping;
	...
	return makeGraph(config).then(function(data){
		...
		// Map our dependencies and add this an option to send into transpile
		if(mapDependencies) {
			eachGraph(dependencyGraph, main, makeDependencyMap);
		}
