...
exports.collect = function({ast}) {
	...
	types.visit(ast, {
		...
		visitExportAllDeclaration(path) {
			let specifier = path.get("source").get("value").value;
			exports.push({ star: true, path, specifier });
...
exports.shakeOut = function(exp, {ast}, edge){
	let {exportName, path} = exp;
	let used = false;
	...
	if(!used) {
		if(exp.star) {
			let specifiers = [];
			for(let exp of edge.usedExports) {
				specifiers.push(
					b.exportSpecifier(b.identifier(exp), b.identifier(exp))
				);
			}

			let expDeclr = b.exportNamedDeclaration(
				null, specifiers, b.literal(exp.specifier)
			);

			path.replace(expDeclr);
		} else {
			path.prune();
		}
		return true;
