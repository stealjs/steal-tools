...
module.exports = function(config, modules, defaults, cb){
	...
	var pluginify,
		pluginifyAndWriteOut = function(moduleNames, out, extraOptions){
			var result = pluginify(moduleNames, _.assign(extraOptions||{},out.output) ),
				filePath;
			...
			if(typeof out.output.dest === "string") {
				...
				filePath = out.output.dest(moduleNames, modulesMap[moduleNames], loads);
			}
	...
	var processOutput = function(out){
		...
		if(out.output.eachModule) {
			...
			eachGraph(pluginify.graph, mods, function(name){
