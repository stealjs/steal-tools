...
var makeBundleGraph = require("../graph/make_graph_with_bundles"),
	...
	directory = require("../bundle/bundles_dir"),
	makeStealNode = require("../node/make_steal_node"),
...
module.exports = function(config, options){
	...
	return makeBundleGraph(config).then(function(data){
		...
		var bundlesDirName = directory.name(options.bundlesDir);
		splitBundles.forEach(function(bundle){
			nameBundle(bundle, bundlesDirName);
		});
		...
		if(options.bundleSteal) {
			masterJS.nodes.unshift(makeNode("preconfig","steal={env: 'production'};"), makeStealNode(),makeDefineNode());
			masterJS.nodes.push( makeNode("import", "System.import('"+main+"');") );
		}
		
		
		// Write bundles to the file system
		return writeBundles(splitBundles, baseURL, options.bundlesDir);
	}).catch(function(e){
