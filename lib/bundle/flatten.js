...
var bundle = {
		...
		merge: function(shares, min){
			...
			winston.debug("Flattening "+shortName(upper.bundles)+">"+
				shortName(lower.bundles)/*+"=" + min.diff._waste*/);
			
			for(var appName in min.diff.bundleToWaste){
				if( min.diff.bundleToWaste[appName] ){
					winston.debug("+ "+min.diff[appName]+" "+shortName([appName]));
				}
			}
		...
		shortName : function(bundles){
			return bundles.map(function(l){
...
module.exports = function(bundles, depth){
	winston.info('Flattening bundles...');
