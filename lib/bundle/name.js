...
function getName(bundle) {
	...
	if(!usedBundleNames[shortened]){
		...
		result = dirName+deNpm(shortened+buildTypeSuffix);
	} else if(shortened.length > 50){
		result = dirName+shortened.substr(0,50)+"-"+bundleNames.length+"-"+(bundleCounter++)+buildTypeSuffix;
	} else {
		result = dirName+shortened+buildTypeSuffix;
	}
