...
describe("multi build", function(){
	...
	describe("multi-main", function(){
		...
		it.only("works with npm plugin using package name", function(done){
			var mains = [
				"multi-main/app_a", "multi-main/app_b",
				"multi-main/app_c", "multi-main/app_d"
			],
				ab = {name: "a_b"},
				cd = {name: "c_d"},
				all = {name: "all"},
			...
			rmdir(__dirname+"/multi-main/dist", function(error){
				...
				multiBuild({
					...
					var checkNext = function(next){
						if(next) {
							next = next.replace("multi-main/", "");
							open("test/multi-main/npm_pkg_"+next+".html",function(browser, close){
								find(browser,"app", function(app){
